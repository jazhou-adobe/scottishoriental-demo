...
The orchestrator dismisses overlays before invoking you and sets consent
cookies, so if the browser session is shared they should not reappear. If you
do see an overlay, click its accept/dismiss button (evaluate the click in the page).

**Wrap page-eval JS in an IIFE** to avoid variable-redeclaration errors across calls.

Evaluate JS in the page to extract the header HTML. Note: some sites use `<nav>`
instead of `<header>` for navigation. Check first to confirm the right element:
...
Crop tight to the header element: screenshot it (~1440px wide) to
`{projectPath}/.migration/source-header.png`. Take an accessibility/DOM
snapshot first if your tool needs an element ref.
...
Read `{projectPath}/blocks/header/header.js` and `{projectPath}/blocks/header/header.css`.
...
- **Logo:** wrap in `<p><a><img></a></p>`, download the image to
  `/drafts/images/` with `curl -L` (never a text-mode file writer — it
  corrupts binary data by coercing bytes to UTF-8)
...
Open `http://localhost:3000/drafts/header-preview.html` in a browser tab. Keep
the tab handle and URL — reload the **same** tab to pick up CSS/JS changes (do
not restart the server per iteration). If the tab is lost, reopen the same URL.
...
```js
JSON.stringify({ hlx: !!window.hlx, codeBasePath: window.hlx?.codeBasePath, bodyAppear: document.body.classList.contains('appear'), headerBlock: !!document.querySelector('.header.block'), navSections: document.querySelectorAll('.header-section').length })
```
...
For thin headers (<150px tall), also use page-evaluated measurements for
...
**Before the first iteration**, note a stable selector (or accessibility ref) for the header element — reuse it for every iteration's crop.

For each iteration:
1. **Screenshot the preview header** by ref (reuse across iterations):
   Screenshot the header element (~1440px wide) to `{projectPath}/.migration/preview-header-iter{N}.png`.
...
4. **Reload:** Refresh to pick up CSS changes (reuse URL from Step 6b — do NOT re-run `serve`):
   Reload the preview tab (same URL) to pick up CSS changes.
