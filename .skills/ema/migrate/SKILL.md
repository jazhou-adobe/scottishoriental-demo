...
1. Run the master skill's setup
   (`slicc-migration/stardust/SKILL.md` § Setup).
...
7. **Validate provenance on every in-scope page.** Call
   ...
   `slicc-migration/stardust/reference/state-machine.md` § Provenance
...
8. **Mobile-adapt audit on every Path A / Path A′ source.** For
   ...
   selection), run the audit per `slicc-migration/prototype/SKILL.md`
   § Mobile-adapt audit:
...
5. Verify **portability**. The bundle must work via `file://`,
   ...
   node slicc-migration/migrate/fixtures/pagemap-audit.mjs stardust/migrated/ stardust/state.json
   ...
   node slicc-migration/migrate/fixtures/file-protocol-audit.mjs stardust/migrated/
...
- Write the top-level `migrate` block per
  `slicc-migration/stardust/reference/migrate-output-format.md`
...
- Affected pages are flagged `stale: true` per
  `slicc-migration/stardust/reference/state-machine.md` § Stale flagging.
...
- `slicc-migration/stardust/reference/migrate-output-format.md` — the
...
- `slicc-migration/stardust/reference/token-contract.md` — `:root` block
  refreshed from DESIGN.md on every render.
- `slicc-migration/stardust/reference/data-attributes.md` — structural
  attributes including `data-template`, `data-module`,
  `data-slot`, `data-canon`, `data-deviation`, `data-bespoke`,
  `data-broken-link`.
- `slicc-migration/stardust/reference/state-machine.md` — page lifecycle,
  page typing, stale-flagging cascade.
- `slicc-migration/stardust/reference/artifact-map.md` — provenance shape
  for migrated artifacts; canon files; sidecar shape.
- `slicc-migration/prototype/reference/canon-extraction.md` — how canon
  is built (input to migrate).
- `slicc-migration/prepare-migration/SKILL.md` — the cascade that
