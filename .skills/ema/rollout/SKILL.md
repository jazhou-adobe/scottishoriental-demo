...
1. Run the master skill's setup (`slicc-migration/stardust/SKILL.md` § Setup).
...
4. If `state.json.handsOff` is true (`slicc-migration/stardust/SKILL.md` § Hands-off
...
```bash
node slicc-migration/rollout/scripts/inventory.mjs --site-url <source-url>
# defaults: --migrated stardust/migrated  --out stardust/rollout
# archetypes-only mode: add the full page roster from state.json
node slicc-migration/rollout/scripts/inventory.mjs --site-url <source-url> --state stardust/state.json
```
...
```bash
node slicc-migration/rollout/scripts/blocks.mjs   # → coverage/blocks.json (the dedup unit)
node slicc-migration/rollout/scripts/plan.mjs     # → plan.json + a readable conversion plan
```
...
2. **Static contract lint (pre-PUT, deterministic).** Before the push, run the
   ...
   ```bash
   node slicc-migration/rollout/scripts/delivery-lint.mjs --file <html> --path </da/path>
   node slicc-migration/rollout/scripts/media-reconcile.mjs --file <html> --deploy-host <branch>--<repo>--<owner>.aem.live [--apply]
   ```
   `media-reconcile` resolves every image on the network and decides
   optimize/keep/rewrite/omit (`slicc-migration/migrate/reference/media-reconciliation.md`)
...
3. **Run the delivery gates** before flipping a page to `deployed`. Each is a
   ...
   - **Source-fidelity** — don't add sections the source lacks; never fabricate
     facts. `node slicc-migration/rollout/scripts/section-fidelity.mjs --file <html> --source <url>`
   ...
   - **Source-content hygiene** — skip dead source URLs; author bodyless/PDF-only
     ...
     `slicc-migration/migrate/reference/fidelity-tiers.md`), don't pad with invented prose.
   - **Fidelity tier declared** — record each page's `fidelityTier`
     ...
     (`slicc-migration/migrate/reference/fidelity-tiers.md`).

4. **Record outcomes** with the state-writer (never hand-edit the ledger):
   ```bash
   node slicc-migration/rollout/scripts/update-coverage.mjs <slug> --status converting
   node slicc-migration/rollout/scripts/update-coverage.mjs --block <id> --status converted --eds-name <name>
   node slicc-migration/rollout/scripts/update-coverage.mjs <slug> --status deployed --url <branch-preview-url>
   node slicc-migration/rollout/scripts/update-coverage.mjs <slug> --status content-pending   # no document push
   ```
...
styled (per `stardust/runtime-contract.json`, `slicc-migration/deploy/SKILL.md`
...
`node slicc-migration/deploy/scripts/deploy-batch.mjs --org <org> --repo <repo>
...
```bash
node slicc-migration/rollout/scripts/assemble.mjs   # → rollout/site/{sitemap.xml,robots.txt,manifest.json}
```
...
```bash
node slicc-migration/rollout/scripts/verify.mjs            # uses rollout.json site.liveHost
...
1. **`rollout:baseline`** — built-in deterministic detectors:
   ```bash
   node slicc-migration/rollout/scripts/optimize.mjs        # uses rollout.json site.liveHost
...
```bash
node slicc-migration/rollout/scripts/findings.mjs record \
...
node slicc-migration/rollout/scripts/findings.mjs resolve <id> --status accepted --note "…"
```
...
```bash
node slicc-migration/rollout/scripts/autofix-aem.mjs --project <eds-root>   # [--dry-run] [--slug s] [--check c]
```
...
`slicc-migration/stardust/reference/learnings.md`: one entry per failure class this run
...
```bash
node slicc-migration/rollout/scripts/dashboard.mjs    # → dashboard/index.html + data.json
```
...
- `slicc-migration/stardust/reference/learnings.md` — the per-run learnings ledger the
  report phase writes.
- `slicc-migration/migrate/reference/fidelity-tiers.md` — the archetype/sibling/thin tier
  contract Phase C records.
- `slicc-migration/migrate/reference/media-reconciliation.md` — the image-fidelity
  resolver's decision table.
- `slicc-migration/deploy/SKILL.md` — the single-page conversion methodology rollout drives.
- `slicc-migration/deploy/da-deploy-protocol.md` — the DA Source API transport.
- `slicc-migration/migrate/SKILL.md` — produces the `migrated/` + `_meta.json` inputs.
