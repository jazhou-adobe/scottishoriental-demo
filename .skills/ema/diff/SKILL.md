...
|---|---|---|---|
| **Pixel / layout** | `slicc-migration/diff/scripts/visual-diff.mjs` | stretched images, dropped max-width wraps, blank renders, surface/ground colour flips, image-count gaps | "right text, wrong slot"; a dropped CTA (full pixels, plausible colours → no flag) |
| **Structural content + type** | `slicc-migration/diff/scripts/content-diff.mjs` | MISSING / ROLE-SWAPPED headings·eyebrows·CTAs, invented/dropped body copy, rendered-FACE font forks (width probe) | geometry / layout regressions |
...
# Copy the WHOLE slicc-migration/diff/scripts/ dir: content-diff imports diff-profiles.mjs AND
...
# 1. PIXEL/layout
node slicc-migration/diff/scripts/visual-diff.mjs   "$PROTO" "$BUILD" --profile eds --sections ".hero"

# 2. STRUCTURAL content + type
node slicc-migration/diff/scripts/content-diff.mjs  "$PROTO" "$BUILD" --profile eds   # --json dumps both inventories
```
...
`slicc-migration/diff/scripts/diff-profiles.mjs` holds them. A profile supplies the source/target **labels**,
...
The structural probe's classifier + differ live in `slicc-migration/diff/scripts/content-inventory.mjs`,
