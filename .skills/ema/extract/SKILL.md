...
(`slicc-migration/stardust/SKILL.md` § Setup): impeccable dep check, context
loader, state read.
...
1. **Playwright availability.** The extraction step needs a real
   ...
   **Bundled crawler.** `slicc-migration/extract/scripts/crawl.mjs` is a
   ...
   (`node slicc-migration/extract/scripts/crawl.mjs --url <origin> [--pages …]
...
- **Hero medium (signature)** — when the hero/first viewport carries a
  *moving* asset (background `<video>` / HLS / canvas / WebGL /
  ...
  preservation (`slicc-migration/stardust/reference/intent-dimensions.md`
...
1. Update `stardust/state.json` (schema in
   `slicc-migration/stardust/reference/state-machine.md`):
...
`slicc-migration/direct/SKILL.md` § Mode A. The donor records
...
`slicc-migration/stardust/reference/state-machine.md` § Page types:
...
- `slicc-migration/stardust/reference/state-machine.md` — state.json contract.
